...
cmd_deps() {
  docker-compose up -d
  echo "已启动 db + redis，可执行 ./run.sh dev 跑后端"
}
...
cmd_dev() {
  ...
  # 开发端口，可通过 PORT=3001 ./run.sh dev 覆盖
  DEV_PORT=${PORT:-3000}
  export PORT=$DEV_PORT
  if port_in_use "$DEV_PORT"; then
    echo "端口 $DEV_PORT 已被占用，无法启动开发服务。"
    echo "  请先执行: ./run.sh down   # 若为 Docker 占用"
    echo "  或使用其他端口: PORT=3001 ./run.sh dev"
    exit 1
  fi
  ...
  if ! command -v air >/dev/null 2>&1; then
    ...
    echo "若已安装，请把 \$(go env GOPATH)/bin 加入 PATH，例如在 ~/.zshrc 中添加："
